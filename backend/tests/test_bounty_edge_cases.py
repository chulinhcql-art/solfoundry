...
class TestComplexFilters:
    ...
    async def test_search_reward_range_exclusive(self, client, db_session):
        ...
        bounties = [
            BountyDB(title="Low", description="D", tier=1, category="backend", status="open", reward_amount=100000.0),
            BountyDB(title="Mid", description="D", tier=1, category="backend", status="open", reward_amount=200000.0),
            BountyDB(title="High", description="D", tier=1, category="backend", status="open", reward_amount=300000.0),
        ]
        for b in bounties:
            db_session.add(b)
        await db_session.commit()
        ...
        response = await client.get("/api/bounties/search?reward_min=100000&reward_max=200000")
...
class TestDefaultBehavior:
    ...
    async def test_search_defaults_to_open_status(self, client, db_session):
        ...
        bounties = [
            ...
            BountyDB(title="Closed Task", description="D", tier=1, category="backend", status="completed", reward_amount=50000.0),
        ]
        for b in bounties:
            db_session.add(b)
        await db_session.commit()
